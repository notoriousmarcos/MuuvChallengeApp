...
class NativeHTTPClientTests: XCTestCase {
    ...
    func testNativeHTTPClient_dispatch_ShouldSuccessWithUsers() {
        // Arrange
        let sut = NativeHTTPClient(session: session)
        let expectedUsers = Mocks.users
        let validData = Mocks.usersData
        ...
        sut.dispatch(request: request) { (result: Result<Users, DomainError>) in
    ...
    func testNativeHTTPClient_dispatch_ShouldReceiveABadRequest() {
        ...
        // Act
        sut.dispatch(request: StubRequest(baseURL: "http\\")) { (result: Result<Users, DomainError>) in
            if case let .failure(error) = result,
